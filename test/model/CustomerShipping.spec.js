/**
 * UltraCart Rest API V2
 * UltraCart REST API Version 2
 *
 * OpenAPI spec version: 2.0.0
 * Contact: support@ultracart.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 2.3.0-SNAPSHOT
 *
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', '../../src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require('../../src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.UltraCartRestApiV2);
  }
}(this, function(expect, UltraCartRestApiV2) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new UltraCartRestApiV2.CustomerShipping();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('CustomerShipping', function() {
    it('should create an instance of CustomerShipping', function() {
      // uncomment below and update the code to test CustomerShipping
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be.a(UltraCartRestApiV2.CustomerShipping);
    });

    it('should have the property address1 (base name: "address1")', function() {
      // uncomment below and update the code to test the property address1
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property address2 (base name: "address2")', function() {
      // uncomment below and update the code to test the property address2
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property city (base name: "city")', function() {
      // uncomment below and update the code to test the property city
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property company (base name: "company")', function() {
      // uncomment below and update the code to test the property company
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property countryCode (base name: "country_code")', function() {
      // uncomment below and update the code to test the property countryCode
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property customerProfileOid (base name: "customer_profile_oid")', function() {
      // uncomment below and update the code to test the property customerProfileOid
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property customerShippingOid (base name: "customer_shipping_oid")', function() {
      // uncomment below and update the code to test the property customerShippingOid
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property dayPhone (base name: "day_phone")', function() {
      // uncomment below and update the code to test the property dayPhone
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property defaultShipping (base name: "default_shipping")', function() {
      // uncomment below and update the code to test the property defaultShipping
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property eveningPhone (base name: "evening_phone")', function() {
      // uncomment below and update the code to test the property eveningPhone
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property firstName (base name: "first_name")', function() {
      // uncomment below and update the code to test the property firstName
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property lastName (base name: "last_name")', function() {
      // uncomment below and update the code to test the property lastName
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property lastUsedDts (base name: "last_used_dts")', function() {
      // uncomment below and update the code to test the property lastUsedDts
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property postalCode (base name: "postal_code")', function() {
      // uncomment below and update the code to test the property postalCode
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property stateRegion (base name: "state_region")', function() {
      // uncomment below and update the code to test the property stateRegion
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property taxCounty (base name: "tax_county")', function() {
      // uncomment below and update the code to test the property taxCounty
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

    it('should have the property title (base name: "title")', function() {
      // uncomment below and update the code to test the property title
      //var instane = new UltraCartRestApiV2.CustomerShipping();
      //expect(instance).to.be();
    });

  });

}));
