...
**item_count** | **Number** | How many entries the menu holds in total, counting every level of the tree. | [optional] 
...
**unconfigured** | **Boolean** | True when a template names this code but no menu has been created for it.  Such a code renders an empty list today.  Write it to create it. | [optional] 
**undeclared** | **Boolean** | True when no template on the storefront names this code.  The menu is stored and editable, but nothing renders it, which usually means a menu element&#39;s menuName is misspelled. | [optional] 
