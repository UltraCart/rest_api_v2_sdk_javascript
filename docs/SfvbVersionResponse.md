...
**max_menu_depth** | **Number** | How deeply store menu entries can nest. | [optional] 
**max_menu_items** | **Number** | Most entries one store menu can hold, counting every level of the tree.  A menu is navigation that renders on every page, so this is deliberately far below what the storage would physically accept. | [optional] 
